...
namespace Soltec.DB
{
    ...
    public class SetDeTransmisionesDB
    {
        ...
        public SetDeTransmisionesDB(IConfiguration configuration, ILogger<SetDeTransmisionesDB> logger)
        {
        ...
        public async Task SincronizaScript_SimiPET(ProcesosOnLine data)
        {
            ...
            try
            {
                Logger.Important($"Iniciando SincronizaScript_SimiPET -- Sucursal: {data.Sucursal}");
                ...
                ventasValidas.ForEach(x => x.IdEmpresa = 0);
                ...
                proceso = " 7) VentasVendedorCuotas";
            ...
            catch (Exception ex)
            {
                try
                {
                    ...
                    Logger.Important($"ERROR EN EL SERVER SIMIPET: {proceso} -  {connString} , {ex.Message}");
                    throw;
                }
        ...
        public async Task SincronizaSetDeTransmisionesSQLServerUltimo(ProcesosOnLine data)
        {
            ...
            if (!ConexionCacheService.TryGetConexion(data.Sucursal?.Trim().ToUpper(), out var conexion))
            {
                Logger.Error($"No existe configuración para ClaveSimi: {data.Sucursal}");
                return;
            }
            ...
            var idEmpresa = conexion.IdEmpresa;

            Logger.Important($"Id Empresa a procesar: {idEmpresa.ToString()}");
            ...
            catch (Exception ex)
            {
                try
                {
                    Logger.Important($"PROCESO_ERROR_{data.Sucursal}: {nombreProceso} - ERROR EN EL SERVER: {conexion.ConnectionString} , {ex.Message}");
                    throw;
                }
                catch (Exception rollEx)
                {

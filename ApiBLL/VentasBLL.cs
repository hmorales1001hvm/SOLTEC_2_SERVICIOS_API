...
namespace ApiBLL
{
    public class VentasBLL
    {
        ...
        public VentasDAL VentasDAL;
        public SetDeTransmisionesDAL SetDeTransmisionesDal;
        

        public VentasBLL(IConfiguration configuration, ILogger<VentasDAL> logger, ILogger<SetDeTransmisionesDAL> logger2)
        {
            Configuration = configuration;
            Logger = logger;
            Logger2 = logger2;
            VentasDAL = new VentasDAL(Configuration, Logger);
            SetDeTransmisionesDal = new SetDeTransmisionesDAL(Configuration, Logger2);
        }

   //     public async Task<List<SqlScripts>> GetSqlScripts()
   //     {
   //         var result = new List<SqlScripts>();
   //         try
   //         {
			//	result = await VentasDAL.GetSqlScriptsDAL();
   ...
   //     }
        ...
        public async Task<List<SPOS_SQLScripts>> GetSQLScriptsSQLite(string numeroSucursal)
        {
            ...
            try
            {
                result = await VentasDAL.GetSQLScriptsSQLite(numeroSucursal);
        ...
        public async Task<List<SPOS_SQLScripts>> ObtieneScripts(string numeroSucursal)
        {
            ...
            try
            {
                result = await VentasDAL.ObtieneScripts(numeroSucursal);
        ...
        public async Task<List<SPOS_SQLScripts>> ObtieneScriptsConCargaInicial(string numeroSucursal)
        {
            ...
            try
            {
                result = await VentasDAL.ObtieneScriptsConCargaInicial(numeroSucursal);
        ...
        public async Task<List<SPOS_SQLScripts>> ObtieneScripts_SIMIPET(string numeroSucursal)
        {
            ...
            try
            {
                result = await VentasDAL.ObtieneScripts_SIMIPET(numeroSucursal);
        ...
        public async Task<List<SPOS_SQLScripts>> GetSPOS_SQLScripts(string numeroSucursal, bool isOnLine)
        {
            ...
            try
            {
				result = await VentasDAL.GetSPOS_SQLScripts(numeroSucursal, isOnLine);
        ...
        public async Task<List<SPOS_SQLScripts>> GetSQLScripts( bool isOnLine, string numeroSucursal)
        {
            ...
            try
            {
                result = await VentasDAL.GetSQLScripts(isOnLine, numeroSucursal);
        ...
        //public async Task OnLineSalesMultiple(ProcesosOnLine data)
        //{
        //    try
        //    {
        //        Logger.LogInformation($"Procesando el proceso ON LINE para : {data.NombreProceso}");
        //        await VentasDAL.OnLineSalesMultiple(data);
        //    }
        //    catch (Exception ex)
        //    {
    ...
    //    }
        ...
        public async Task<byte[]> GetReporteExcel(string rfcEmpresa)
        {
            try
            {
                ...
                var getReporteExcelData = await VentasDAL.GetReporteExcelDataDAL(rfcEmpresa);
        ...
        public async Task<ParametrosGenerales> GetParametros(string claveSimi)
        {
            ...
            try
            {
				...
				result = await VentasDAL.GetParametrosDAL(claveSimi);
